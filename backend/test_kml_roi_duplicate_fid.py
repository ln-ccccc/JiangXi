"""同 fid 多 Placemark 的数据正确性测试（2026-09-14 复审批次 F1）。
...
硬性约束：fid 唯一（348 主链路）时行为与历史完全一致，
既有 test_kml_roi_raster_ops / test_spectral_indices 中的相关用例即为其回归门。
"""
...
class FilterFeaturesByBoundsDuplicateFidTests(unittest.TestCase):
    """F1a：同 fid 多 Placemark 不得被空间过滤丢弃，两条路径行为一致。"""

    BOUNDS = (116.0, 28.3, 116.2, 28.48)

    def _features(self):
        return [
            ("23", _polygon(116.02, 28.32, 116.08, 28.38)),  # 相交，第 1 个图斑
            ("23", _polygon(116.12, 28.40, 116.18, 28.46)),  # 相交，同 fid 第 2 个图斑
            ("24", _polygon(116.02, 28.32, 116.08, 28.38)),  # 相交，其他 fid
            ("25", _polygon(116.30, 28.30, 116.40, 28.40)),  # 不相交
    ...
    def test_unique_fid_behavior_unchanged(self):
        features = [
            ("23", _polygon(116.02, 28.32, 116.08, 28.38)),
            ("24", _polygon(116.12, 28.40, 116.18, 28.46)),
            ("25", _polygon(116.30, 28.30, 116.40, 28.40)),
        ]
...
class PrepareTilesDuplicateFidTests(unittest.TestCase):
    """F1b：同 fid 多 variant 的瓦片产物必须可区分且不互相覆盖。"""
    ...
    def test_same_fid_variants_get_distinct_names(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            features = [
                ("23", _polygon(116.02, 28.32, 116.08, 28.38)),
                ("23", _polygon(116.12, 28.40, 116.18, 28.46)),
            ]
            with patch(
...
class MergeKmlDuplicateFidTests(unittest.TestCase):
    """F1c：base 内同 fid 多 Placemark 必须全部参与合并，结果无重复 fid。"""
...
class DuplicateFidPipelineTests(unittest.TestCase):
    """F1 端到端：同 fid 双 Placemark KML，两个图斑都被切片/推理/落盘。
    ...
    def _write_tif(path, value):
        ...
        profile = {
            ...
            "dtype": "uint8",
            "crs": "EPSG:4326",
            "transform": from_origin(116.0, 28.5, 0.001, 0.001),
        }
    ...
    def test_both_placemarks_tiled_inferred_and_written(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            kml_path.write_text(
                _kml_document(
                    _placemark_xml("23", 116.02, 28.32, 116.08, 28.38)
                    + _placemark_xml("23", 116.12, 28.40, 116.18, 28.46)
                ),
                encoding="utf-8",
            )
            ...
            with patch(
                ...
                side_effect=fake_run_mmseg_tiles,
            ):
