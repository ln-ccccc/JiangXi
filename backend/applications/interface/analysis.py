...
def _mine_boundary_stats(src, input_path, index_data, valid_mask, kml_path=None, fid=None):
    if src.crs is None:
        return np.array([], dtype=np.float32), "raster_missing_crs", 0, []

    resolved_kml = Path(kml_path).expanduser() if kml_path else _default_kml_path()
    if not resolved_kml.exists():
        return np.array([], dtype=np.float32), "kml_missing", 0, []

    features = load_kml_features(resolved_kml)
    bounds_4326 = raster_bounds_4326(Path(input_path))
    features = filter_features_by_bounds(features, bounds_4326)
    if fid:
        fid_text = str(fid)
        features = [(f, geom) for f, geom in features if str(f) == fid_text]
    if not features:
        ...
        data = json.dumps(
            {
                ...
                "mean": _safe_mean(boundary_values),
                "mean_scope": "mine_boundary",
