...
def execute_mmseg(model_path, data_path, out_dir, test_names, device="auto"):
...
def execute(model_path, data_path, out_dir, test_names, device="auto"):
    model_id = get_mmseg_model_id(model_path)
    print(f"[SemanticSegmentation] 使用 MMSegmentation 模型: {model_id}", flush=True)
    return execute_mmseg(model_path, data_path, out_dir, test_names, device=device)
