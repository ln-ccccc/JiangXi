...
def _project_output_directory(category, project_id):
    root = Path(current_app.config["PROJECT_OUTPUT_ROOT"]).resolve() / category
    return _ensure_dir(resolve_output_directory(root, project_id))


def _backup_manifest_path(project_id, manifest_path):
    backup_dir = _project_output_directory("backups", project_id)
    stored_path = Path(manifest_path).expanduser().resolve()
    candidate = resolve_managed_file(backup_dir, stored_path.name, {".json"})
    if stored_path != candidate:
        raise PathValidationError("备份文件不在受控目录")
    return candidate
...
def create_export(project_id, payload):
    project = _get_project_or_404(project_id)
    if payload.get("output_dir") not in (None, ""):
        raise ValueError("不支持自定义输出目录")
    ...
    output_dir = _project_output_directory("exports", project_id)
...
def create_backup(project_id, payload):
    ...
    output_dir = _project_output_directory("backups", project_id)
...
def restore_backup(project_id, backup_id):
    ...
    manifest_path = _backup_manifest_path(project.id, backup.manifest_path)
