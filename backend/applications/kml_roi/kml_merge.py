...
def merge_kml_increment(base_kml: Path, incoming_kml: Path, output_kml: Optional[Path] = None) -> Dict:
    if not incoming_kml.exists():
        raise FileNotFoundError(f"kml_path not found: {incoming_kml}")

    if base_kml.exists():
        base_tree = ET.ElementTree(load_kml_root(base_kml))
    else:
        base_tree = _new_doc()
    incoming_tree = ET.ElementTree(load_kml_root(incoming_kml))
