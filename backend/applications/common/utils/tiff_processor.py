...
def _border_connected_zero_mask(zero_mask: np.ndarray) -> np.ndarray:
    """Find zero-value area connected to image border."""
    h, w = zero_mask.shape
    visited = np.zeros_like(zero_mask, dtype=np.uint8)
    q = deque()
    ...
    for x in range(w):
        if zero_mask[0, x]:
            visited[0, x] = 1
            q.append((0, x))
        if zero_mask[h - 1, x] and not visited[h - 1, x]:
            visited[h - 1, x] = 1
            q.append((h - 1, x))
    for y in range(h):
        if zero_mask[y, 0] and not visited[y, 0]:
            visited[y, 0] = 1
            q.append((y, 0))
        if zero_mask[y, w - 1] and not visited[y, w - 1]:
            visited[y, w - 1] = 1
            q.append((y, w - 1))
    ...
    while q:
        ...
        for ny, nx in ((cy - 1, cx), (cy + 1, cx), (cy, cx - 1), (cy, cx + 1)):
            if 0 <= ny < h and 0 <= nx < w and zero_mask[ny, nx] and not visited[ny, nx]:
                visited[ny, nx] = 1
                q.append((ny, nx))
    return visited.astype(bool)


def _build_invalid_mask_from_raster(src, raw_data: np.ndarray, rgb_data: np.ndarray) -> np.ndarray:
    """Build invalid area mask; return None when raster metadata is unavailable."""
    invalid_mask = np.zeros((rgb_data.shape[0], rgb_data.shape[1]), dtype=bool)
    has_raster_mask = False
    try:
        band_mask = src.read_masks(1)
        if band_mask is not None:
            band_invalid = (band_mask == 0)
            if np.any(band_invalid):
                invalid_mask |= band_invalid
                has_raster_mask = True
    except Exception:
        pass

    if src.nodata is not None:
        try:
            nodata_mask = np.all(raw_data == src.nodata, axis=2)
            if np.any(nodata_mask):
                invalid_mask |= nodata_mask
                has_raster_mask = True
        except Exception:
            pass
    ...
    # fallback for files without nodata/mask metadata
    zero_mask = np.all(rgb_data == 0, axis=2)
    if np.any(zero_mask):
        return _border_connected_zero_mask(zero_mask)
    return invalid_mask
...
def read_tiff_as_rgb(tiff_path: str) -> np.ndarray:
    ...
    try:
        ...
        with rasterio.open(tiff_path) as src:
            if raw_data is None:
                raw_data = data
            invalid_mask = _build_invalid_mask_from_raster(src, raw_data, rgb_data)
    except Exception:
        invalid_mask = None

    # RGB-only strategy: invalid pixels use white background.
    if invalid_mask is not None and np.any(invalid_mask):
        rgb_data[invalid_mask] = 255
