...
test('resolveLocalTilePath rejects traversal coordinate values', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'tile-root-'));
  const outsideDir = path.join(path.dirname(root), 'outside-tile');
  fs.mkdirSync(outsideDir, { recursive: true });
  fs.writeFileSync(path.join(outsideDir, '433.png'), Buffer.from('private'));

  assert.equal(resolveLocalTilePath(root, '..', 'outside-tile', '433'), null);
});
