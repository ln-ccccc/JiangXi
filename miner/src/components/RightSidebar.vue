...
const visiblePanels = computed(() => (props.recommendedPanels || [])
  .filter((panel) => Array.isArray(panel?.data) && panel.data.length > 0));
  ...
  series: [{
    ...
    itemStyle: {
      color: new echarts.graphic.LinearGradient(1, 0, 0, 0, [
        { offset: 0, color: '#78a99e' },
        { offset: 0.62, color: '#4f8987' },
        { offset: 1, color: '#a87859' }
      ])
    }
  }]
});
...
.glass-panel {
  background: rgba(13, 32, 44, 0.64);
  border: 1px solid rgba(139, 180, 171, 0.15);
  border-radius: 6px;
