<template>
  ...
  <div v-if="showPanel" class="fixed bottom-5 right-5 w-[480px] bg-white shadow-lg rounded-lg p-3">
    <h3 class="text-lg font-bold mb-2">图斑 {{ currentTBBH }}</h3>
    <div class="border-b mb-2 flex space-x-4 text-sm">
      <button
        class="pb-1 border-b-2"
        :class="
          tab === 'matrix' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500'
        "
        @click="tab = 'matrix'"
      >
        变化矩阵
    ...
    <div v-else class="text-sm">
      <div v-if="indexSeries.years.length === 0">暂无多年份指标数据</div>
      <div v-else>
        <div class="flex items-center mb-2">
          <span class="mr-2">指标：</span>
          <select v-model="selectedMetric" class="border rounded px-2 py-1 text-sm">
        ...
        <div class="mt-2 text-xs text-gray-500">年份: {{ indexSeries.years.join(', ') }}</div>
      </div>
    </div>
  </div>
</template>
...
onMounted(async () => {
  const map = L.map('map').setView(JIANGXI_FALLBACK_CENTER, JIANGXI_FALLBACK_ZOOM);
  const provider = normalizeMapProvider(import.meta.env.VITE_MINER_MAP_PROVIDER);
    ...
    polygon.on('click', async () => {
      ...
      try {
        const idxRes = await axios.get(
          apiUrl(`/api/mines/indices?tbbh=${encodeURIComponent(tbbh)}`)
        );
        const d = idxRes.data;
  ...
  const bounds = mineGroup.getBounds();
  if (bounds.isValid()) {
