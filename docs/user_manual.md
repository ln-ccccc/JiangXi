...
江西默认执行方式为同步 CPU。页面不查询异步任务，不提供自动设备选择，也不提供 GPU 异常时自动回退 CPU 的提示或能力。

解译平台中的“返回矿山地图”只返回 `http://127.0.0.1:4173/#/map`。缺少 Miner 地址配置时会报错，不使用替代地址。

## 6. 可选 GPU 文件

`docker-compose.gpu.yml` 仅供运维人员在具备 `jiangxi-runtime:gpu`、驱动和单独验证计划时叠加。普通用户和默认部署不使用该文件。叠加它不代表系统具备自动设备选择或异常回退。
