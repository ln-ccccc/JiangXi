...
test("地物分类渲染真实运行状态而不是静态状态说明", () => {
    ...
    assert.match(
        segmentation,
        /analysisRunMessage:\s*"请先选择 tif \/ tiff 影像。"/
    );
    assert.match(
        runPanel,
...
test("地物分类工具链按真实 Promise 分支更新 running、partial、success 与 error", () => {
    const runningIndex = uploadUtility.search(
    ...
    assert.match(
        uploadUtility,
        /if \(totalFailureCount > 0\) \{[\s\S]*setAnalysisRunState\(\s*this,\s*["']partial["']/
    );
    assert.match(
        uploadUtility,
        /\} else \{[\s\S]*setAnalysisRunState\(\s*this,\s*["']success["']/
    );
    assert.match(
        uploadUtility,
        /rawTiffPaths\.length === 0[\s\S]*setAnalysisRunState\(\s*this,\s*["']error["']/
    );
    assert.match(
        uploadUtility,
        /Flash 推理失败，未生成任何结果[\s\S]*setAnalysisRunState\(\s*this,\s*["']error["']/
    );
    assert.match(
        uploadUtility,
        /\.catch\(\(err\) => \{[\s\S]*setAnalysisRunState\(\s*this,\s*["']error["']/
    );
...
test("flash 卡片 record_id 冻结为 2 段式 tbbh|name（与后端删除接口对齐）", () => {
    // 后端 /api/analysis/kml_roi_history/item 按首末段解析 record_id（兼容 2/3 段式），
...
test("预处理预览失败必须给出可见提示而不是静默吞掉", () => {
    ...
    assert.match(
        preHandleUtility,
        /\$message\?\.\s*warning\?\.\(\s*['"]预处理预览生成失败/
    );
    assert.match(
        preHandleUtility,
        /\$message\?\.\s*warning\?\.\(\s*['"]预处理原图上传失败/
    );
});
