    ...
    <div class="img-display-box">
      ...
      >
        <el-divider class="img-divider">
          第<span class="index-number">{{ item.display_index || (index + 1) }}</span>组
          <span
            class="delete-item"
          ...
          <div v-if="item.type!=='场景分类'">
            <div style="display: flex;">
              <div>
                ...
                <div
                  v-if="item.data && item.data.index_type"
                  class="spectral-stat"
                >
                  <div>{{ item.data.index_type }} 矿山边界内平均值：{{ formatNumber(item.data.mean) }}</div>
                  <div v-if="item.data.valid_pixel_count !== undefined">
                    有效像元：{{ item.data.valid_pixel_count }}
                  </div>
                  <div v-if="item.data.boundary_status && item.data.boundary_status !== 'ok'">
                    边界状态：{{ boundaryStatusText(item.data.boundary_status) }}
                  </div>
                </div>
              </div>
